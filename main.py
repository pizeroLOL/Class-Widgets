...
logger.add(
    "log/ClassWidgets_main_{time}.log",
    rotation="10 MB",
    encoding="utf-8",
    retention="2 days",
)
...
def get_next_lessons():
    ...
    if afternoon_st != 0 and current_dt > afternoon_st - dt.timedelta(
        minutes=30
    ):  # 提前30min获取下午课程
...
def get_current_state():
    ...
    # 上午
        ...
        c_time = morning_st + dt.timedelta(
            seconds=time_offset
        )  # 复制 afternoon_st 时间
        for item_name, item_time in timeline_data.items():
            if item_name.startswith("am") or item_name.startswith("fm"):
...
class DesktopWidget(QWidget):  # 主要小组件
    def __init__(self, path="widget-time.ui", pos=(100, 50), enable_tray=False):
        ...
        match path:
            ...
            case "widget-countdown.ui":  # 活动倒计时
                self.countdown_progress_bar = self.findChild(
    ...
    def animate_window(self, target_pos):  # 窗口动画！
        ...
        self.animation.setStartValue(
            QRect(target_pos[0], -self.height(), self.width(), self.height())
        )
        self.animation.setEndValue(
            QRect(target_pos[0], target_pos[1], self.width(), self.height())
        )
    ...
    def animate_auto_hide(self):  # 自动隐藏窗口
        ...
        self.animation.setEndValue(
            QRect(self.x(), -self.height() + 40, self.width(), self.height())
        )
        self.animation.setEasingCurve(QEasingCurve.Type.InOutCirc)  # 设置动画效果
        self.animation.start()
    ...
    def update_data(self, first_setup=0, path=""):
        ...
        bkg.setStyleSheet(
            f"background-color: rgba(242, 243, 245, {int(transparent)}); border-radius: 8px"
        )  # 背景透明度
