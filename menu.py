...
class desktop_widget(FluentWindow):
    ...
    def setup_customization_interface(self):
        ...
        widgets_list.addItems(
            (list.widget_name[key] for key in list.get_widget_config())
        )

        switch_countdown_custom = self.findChild(
            SwitchButton, "switch_countdown_custom"
        )
        switch_countdown_custom.setChecked(
            conf.get_is_widget_in("widget-countdown-custom.ui")
        )
    ...
    def setup_advance_interface(self):
        ...
        conf_combo.setCurrentIndex(
            list.get_schedule_config().index(conf.read_conf("General", "schedule"))
        )
        ...
        switch_enable_alt_schedule = self.findChild(
        ...
        switch_enable_alt_schedule.checkedChanged.connect(
            self.switch_enable_alt_schedule
        )  # 单双周开关
    ...
    def ct_save_widget_config(self):
        ...
        for i in range(widgets_list.count()):
            widget_config["widgets"].append(
                list.widget_conf[widgets_list.item(i).text()]
            )
    ...
    def ad_change_file_name(self):
        ...
        try:
            ...
            os.rename(
                f"config/schedule/{old_name}", f"config/schedule/{new_name}.json"
            )  # 重命名
