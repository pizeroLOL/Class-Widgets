...
class CSES_Converter:
    ...
    def convert_to_cw(self) -> Union[Dict, bool]:
        ...
        """
        try:
            with open(f"{base_directory}/config/default.json", encoding="utf-8") as file:  # 加载默认配置
    ...
    def convert_to_cses(self, cw_data: Optional[Dict[str, Any]] = None, cw_path: str = "./") -> bool:
        ...
        # 科目
        try:
            with open(f"{base_directory}/config/data/subject.json", encoding="utf-8") as data:
        ...
        for subject_ in cw_subjects["subject_list"]:
            self.generator.add_subject(
                name=subject_, simplified_name=list_.get_subject_abbreviation(subject_), teacher=None, room=None
            )
