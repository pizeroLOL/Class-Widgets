...
version_channel = [
    ...
    QCoreApplication.translate("list_", "测试版 (Beta)"),
]
...
default_widgets = ["widget-time.ui", "widget-countdown.ui", "widget-current-activity.ui", "widget-next-activity.ui"]
...
try:  # 加载课程/主题配置文件
    subject_info = json.load(open(f"{base_directory}/config/data/subject.json", encoding="utf-8"))
...
def get_widget_config() -> List[str]:
    try:
        if os.path.exists(f"{base_directory}/config/widget.json"):
            with open(f"{base_directory}/config/widget.json", encoding="utf-8") as file:
                data = json.load(file)
        else:
