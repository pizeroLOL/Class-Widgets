            ...
            if daily_data:
                ...
                if len(daily_data) > 1:
                    ...
                    try:  #                            ↑ ↑ ↑
                        ...
                        tomorrow_high = float(
                            tomorrow.get("temp_low", tomorrow.get("tempMax", tomorrow.get("daytemp", 0)))
                        )
                                ...
                                reminders.append(
                                    {
                                        ...
                                        "title": QCoreApplication.translate("WeatherReminder", "降水将持续很久"),
                                        "icon": "rain",
                                    }
                                )
                            ...
                            reminders.append(
                                {
                                    ...
                                    "title": QCoreApplication.translate("WeatherReminder", "{} 小时后有降水").format(
                                        hours
                                    ),
                                    "icon": "rain",
                                }
                            ...
                            reminders.append(
                                {
                                    ...
                                    "title": QCoreApplication.translate("WeatherReminder", "明日有降水"),
                                    "icon": "rain",
                                }
                            )
    ...
    def parse_daily_forecast(self, data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if tomorrow_precipitation:
                ...
                # 添加统计结果
                result.append(
                    {"tomorrow_precipitation": tomorrow_precipitation, "precipitation_day": precipitation_day}
                )
...
class OpenMeteoProvider(GenericWeatherProvider):
    ...
    def fetch_forecast_data(self, location_key: str, api_key: str, forecast_type: str, days: int = 7) -> Dict[str, Any]:
        ...
        try:
            ...
            headers = {"User-Agent": "ClassWidgets/1.1.7.2 (contact: IsHPDuwu@outlook.com)"}
        ...
        try:
            ...
            for i in range(min(len(times), 24)):  # 限制为24小时
                hour_data = {
                    ...
                    "temperature": f"{temperatures[i]}{hourly_units.get('temperature_2m', '°C')}"
                    if i < len(temperatures) and temperatures[i] is not None
                    else None,
                    "weather_code": str(weather_codes[i])
                    if i < len(weather_codes) and weather_codes[i] is not None
                    else None,
                    "apparent_temperature": f"{apparent_temps[i]}{hourly_units.get('apparent_temperature', '°C')}"
                    ...
                    "wind_speed": f"{wind_speeds[i]} {hourly_units.get('wind_speed_10m', 'km/h')}"
                    if i < len(wind_speeds) and wind_speeds[i] is not None
                    else None,
                    "wind_direction": f"{wind_directions[i]}°"
                    if i < len(wind_directions) and wind_directions[i] is not None
                    else None,
                    "pressure": f"{pressures[i]} {hourly_units.get('surface_pressure', 'hPa')}"
                    if i < len(pressures) and pressures[i] is not None
                    else None,
                    "visibility": f"{visibility[i] / 1000:.1f} km"
        ...
        try:
            ...
            for i in range(min(len(times), 7)):  # 限制为7天
                day_data = {
                    ...
                    "temp_max": f"{temp_max[i]}{daily_units.get('temperature_2m_max', '°C')}"
                    if i < len(temp_max) and temp_max[i] is not None
                    else None,
                    "temp_min": f"{temp_min[i]}{daily_units.get('temperature_2m_min', '°C')}"
...
class WeatherDataProcessor:
    ...
    def _load_weather_status(self, api_name: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            with open(os.path.join(base_directory, "config", "data", f"{api_name}_status.json"), encoding="utf-8") as f:
    ...
    def _extract_amap_value(self, context: WeatherExtractionContext) -> str:
        """提取高德天气值"""
        return context.weather_data.get("lives", [{}])[0].get(context.current_params.get(context.key, ""), "")
    ...
    def _extract_qq_value(self, context: WeatherExtractionContext) -> str:
        ...
        if realtime_data:
            return str(realtime_data[0].get("infos", {}).get(context.current_params.get(context.key, ""), ""))
        return ""
...
def get_alert_icon_by_severity(severity: Union[str, int]) -> str:
    ...
    try:
        ...
        return (
            ...
            else os.path.join(base_directory, "img", "weather", "alerts", "blue.png")
        )
    except Exception as e:
...
def get_hourly_forecast() -> Dict[str, Any]:
    ...
    try:
        ...
        if not hourly_supported:
            ...
            return {"success": False, "supported": False, "data": [], "error": f"{provider.api_name} 不支持逐小时预报"}

        forecast_data = weather_manager.fetch_hourly_forecast()
        return {"success": True, "supported": True, "data": forecast_data if forecast_data else [], "error": None}
...
def get_daily_forecast(days: int = 5) -> Dict[str, Any]:
    ...
    try:
        provider = weather_manager.get_current_provider()
        if not provider:
            return {"success": False, "supported": False, "data": [], "days": 0, "error": "未找到天气提供者"}
        # 检查是否支持多天预报
        forecast_config = provider.config.get("forecast", {})
