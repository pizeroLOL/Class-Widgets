...
class TTSCache:
    ...
    def _cleanup_if_needed(self) -> None:
        ...
        if len(self._cache_info) <= self.max_size:
            return
        sorted_files = sorted(self._cache_info.items(), key=lambda x: x[1]["accessed_at"])
            ...
            def _run_synthesis():
                ...
                try:
                    ...
                    if not os.path.exists(output_path):
                        raise RuntimeError(
                            QCoreApplication.translate("EdgeTTSProvider", "语音文件生成失败，文件不存在")
                        )
                    ...
                    if "No audio was received" in error_msg:
                        raise RuntimeError(
                            QCoreApplication.translate(
                                "EdgeTTSProvider", "Edge TTS服务未返回音频数据,可能是网络问题或语音参数错误。语音ID: {}"
                            ).format(voice_id)
                        )
                    if "proxy" in error_msg.lower() or "https" in error_msg.lower():
                        raise RuntimeError(
                            QCoreApplication.translate("EdgeTTSProvider", "连接问题,可能是代理设置导致: {}").format(
                                error_msg
                            )
                        )
                    if "timeout" in error_msg.lower():
                        raise RuntimeError(
                            QCoreApplication.translate("EdgeTTSProvider", "超时,请检查网络连接: {}").format(error_msg)
                        )
                    raise RuntimeError(
                        QCoreApplication.translate("EdgeTTSProvider", "Edge TTS合成失败: {}").format(error_msg)
                    )
                finally:
                    self._safe_cleanup_loop(loop)
    ...
    def _fetch_voices(self) -> List[TTSVoice]:
        ...
        try:
            ...
            for voice in voices:
                voice_obj: Any = voice
                language = (
                    "zh" if any(keyword in voice_obj.name.lower() for keyword in ["chinese", "zh", "中文"]) else "en"
                )
                tts_voice = TTSVoice(
                    id=voice_obj.id, name=voice_obj.name, language=language, gender="unknown", engine=TTSEngine.PYTTSX3
                )
                result.append(tts_voice)
...
class TTSManager:
    ...
    def get_voices(self, engine: Optional[TTSEngine] = None, language_filter: Optional[str] = None) -> List[TTSVoice]:
        """获取语音列表"""
        if engine:
    ...
    def get_available_engines(self) -> Dict[TTSEngine, str]:
        """获取可用的 TTS 引擎"""
        engines = {TTSEngine.EDGE: "Edge TTS"}
        if platform.system() == "Windows":
    ...
    def _synthesize_speech(
        self, text: str, engine: TTSEngine, voice_id: Optional[str], output_path: str, speed: float, auto_fallback: bool
    ) -> bool:
        """合成语音"""
            ...
            if auto_fallback:
                for fallback_engine, fallback_provider in self.providers.items():
                    if fallback_engine != engine:
                        ...
                        if voices:
                            return fallback_provider.synthesize(text, voices[0].id, output_path, speed)
            return False
...
def generate_speech_sync(
    ...
    file_path = manager.generate_speech(
        text=text, engine=engine_enum, voice_id=voice_id, speed=speed, auto_fallback=auto_fallback
    )
    if not file_path:
...
class TTSService(QObject):
    ...
    def __init__(self):
        if TTSService._instance is not None:
            raise RuntimeError(
                QCoreApplication.translate("TTSService", "热芝士: 使用 TTSService.get_instance() 获取实例")
            )
        ...
        def _generate_in_background():
            try:
                ...
                file_path = self._manager.generate_speech(
                    text=text, engine=engine, voice_id=voice_id_only, speed=speed, auto_fallback=auto_fallback
                )
                with self._generation_lock:
                    if not self._active_generations.get(task_id, False):
                        return
...
def generate_tts_sync(
    text: str, voice_id: Optional[str] = None, speed: float = 1.0, auto_fallback: bool = False, timeout: float = 15.0
) -> Optional[str]:
    """
    同步生成TTS语音文件
...
def play_tts_with_audio(
    text: str,
    voice_id: Optional[str] = None,
    speed: float = 1.0,
    auto_fallback: bool = False,
    tts_delete_after: bool = True,
...
async def get_tts_voices(
    engine_filter: Optional[str] = None, language_filter: Optional[str] = None
