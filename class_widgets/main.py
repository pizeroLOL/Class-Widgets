...
if config_center.read_conf("Other", "do_not_log") != "1":
    logger.add(
        f"{base_directory}/log/ClassWidgets_main_{{time}}.log", rotation="1 MB", encoding="utf-8", retention="1 minute"
    )
    logger.info("未禁用日志输出")
else:
...
def get_part() -> Optional[Tuple[dt.datetime, int]]:
    ...
    def return_data():
        ...
        c_time = current_manager.get_current_time().replace(
            hour=base_time.hour, minute=base_time.minute, second=base_time.second, microsecond=base_time.microsecond
        )
    ...
    for i in range(len(parts_start_time)):  # 遍历每个Part
        ...
        for item_name, item_time in timeline_data.items():
            if item_name.startswith(f"a{order[i]!s}") or item_name.startswith(f"f{order[i]!s}"):
...
def get_countdown(toast: bool = False) -> Optional[List[Union[str, int]]]:  # 重构好累aaaa
    ...
    if parts_start_time:
        ...
        if current_dt >= c_time:
            for item_name, item_time in timeline_data.items():
                if item_name.startswith(f"a{part!s}") or item_name.startswith(f"f{part!s}"):
                    # 判断时间是否上下课，发送通知
                    ...
                    if (
                        current_dt
                        == c_time - dt.timedelta(minutes=int(config_center.read_conf("Toast", "prepare_minutes")))
...
class PluginManager:  # 插件管理器
    ...
    def get_app_contexts(self, path: Optional[str] = None) -> Dict[str, Any]:
        self.cw_contexts = {
            ...
            "PLUGIN_PATH": os.path.normpath(os.path.join(conf.PLUGINS_DIR, path))
            if path
            else conf.PLUGINS_DIR,  # 传递插件目录
...
class PluginMethod:  # 插件方法
    ...
    @staticmethod
    def generate_speech(
        text: str, engine: str = "edge", voice: Optional[str] = None, timeout: float = 10.0, auto_fallback: bool = True
    ) -> str:
        """
        同步生成语音文件（供插件调用）
...
class WidgetsManager:
    ...
    def get_widget_width(path: str) -> int:
        return load_theme_config(str("default" if theme is None else theme)).config.widget_width.get(
            path, list_.widget_width.get(path, 0)
        )
    ...
    def get_widget_pos(self, path: str, cnt: Optional[int] = None) -> List[int]:  # 获取小组件位置
        ...
        for i in range(num):
            ...
            pos_x += conf.load_theme_config(str("default" if theme is None else theme)).config.widget_width.get(
                widget, list_.widget_width.get(widget, 0)
            )
...
class openProgressDialog(QWidget):
    def __init__(self, action_title="打开 记事本", action="notepad"):
        super().__init__()
        self.setWindowFlags(
            self.windowFlags() | Qt.WindowStaysOnTopHint | Qt.WindowType.WindowDoesNotAcceptFocus | Qt.Tool
        )
    ...
    def intro_animation(self):  # 弹出动画
        ...
        self.animation_rect.setStartValue(QRect(self.x(), self.screen_height, self.width(), self.height()))
...
class FloatingWidget(QWidget):  # 浮窗
            ...
            try:
                ...
                if ctypes.windll.user32.IsWindow(hwnd):
                    ...
                    ctypes.windll.user32.SetWindowPos(
                        hwnd, HWND_TOPMOST, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOACTIVATE | SWP_NOSIZE | SWP_SHOWWINDOW
                    )
                    self.raise_()
    ...
    def showEvent(self, event: QShowEvent) -> None:  # 窗口显示
        ...
        if self.position:
            ...
            # 默认:顶部中央滑入
            start_pos = QPoint((screen_geometry.width() - self.width()) // 2, screen_geometry.top() - self.height())
            self.position = QPoint(
    ...
    def closeEvent(self, event: QCloseEvent) -> None:
        ...
        if main_widget:
            ...
            target_pos = QPoint(
                screen_geometry.center().x() - self.width() // 2, int(config_center.read_conf("General", "margin"))
            )
            distance = abs(current_pos.y() - target_pos.y())
    ...
    def mouseReleaseEvent(self, event: QMouseEvent) -> None:
        ...
        if hide_mode == "1" or hide_mode == "2":
            return  # 阻止手动展开/收起
        if (
            hasattr(self, "p_Position") and self.r_Position == self.p_Position and not self.animating
        ):  # 非特定隐藏模式下执行点击事件
            if hide_mode == "3":
                if mgr.state:
                    mgr.decide_to_hide()
...
class DesktopWidget(QWidget):  # 主要小组件
    def __init__(
        ...
        if path == "widget-time.ui":  # 日期显示
            ...
            self.date_text.setText(
                self.tr("{year} 年 {month}").format(year=today.year, month=list_.month[today.month - 1])
            )
    ...
    def apply_window_state(self) -> None:
        ...
        def apply_state_changes():
            ...
            if pin_on_top == "1":  # 置顶
                ...
                new_flags = Qt.WindowType.FramelessWindowHint | Qt.WindowType.WindowDoesNotAcceptFocus | Qt.Tool
            else:
    ...
    def animate_expand(self, target_geometry: QRect) -> None:
        self.animation = QPropertyAnimation(self, b"geometry")
        self.animation.setDuration(400)
        self.animation.setStartValue(QRect(target_geometry.x(), -self.height(), self.width(), self.height()))
        self.animation.setEndValue(target_geometry)
    ...
    def update_data(self, path: str = "") -> None:
        ...
        if path == "widget-current-activity.ui":  # 当前活动
            ...
            if current_state != 2:  # 非休息段
                ...
                self.blur_effect_label.setStyleSheet(f"background-color: rgba{list_.subject_color('课间')}, 200);")
                ...
                if config_center.read_conf("General", "blur_countdown") == "1":  # 模糊倒计时
                    if cd_list[1] == "00:00":
                        ...
                        self.activity_countdown.setText(
                            self.tr("< {minutes} 分钟").format(minutes=int(cd_list[1].split(":")[0]) + 1)
                        )
                else:
                    self.activity_countdown.setText(cd_list[1])
        ...
        if type(weather_data) is dict and hasattr(self, "weather_icon") and "error" not in weather_data:
            ...
            try:
                ...
                if city_name != "coordinates":
                    current_city.setText(f"{city_name} · {weather_name}")
                else:
                ...
                update_stylesheet = re.sub(
                    r"border-image: url\([^)]*\);", f"border-image: url({path});", self.backgnd.styleSheet()
                )
    ...
    def widget_transition(
        self, pos_x: int, width: int, height: int, opacity: float = 1, pos_y: int = None
    ) -> None:  # 窗口形变
        self.animation = QPropertyAnimation(self, b"geometry")
        self.animation.setDuration(525)  # 持续时间
    ...
    for window in all_windows:
        try:
            ...
            if process_name == "explorer.exe":
                if title_lower in excluded_titles_exact_lower or any(
                    keyword in title_lower for keyword in excluded_keywords_in_title_lower
                ):
...
def main():
    ...
    if scale_factor > 1.8 or scale_factor < 1.0:
        ...
        msg_box = Dialog(
            ...
            QCoreApplication.translate(
                "main", "当前缩放系数为 {scale_factor}%，可能导致显示异常。\n建议将缩放系数设置为 100% 到 180% 之间。"
